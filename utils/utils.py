...
# have the label cubix box
class CropResize(BaseTransform):
    def __init__(self, fields, output_size,pad=32, shift=0, shift_L=0, shift_W=0, shift_H=0):
        """
        Args:
            fields: fields specifying image paths to load
            root_dir: root dir of images
        ...
        for f in self.fields:
            if data_dict.get(f) is not None:
                if isinstance(data_dict.get(f+'_pancreas_pred') ,np.ndarray):
                    ...
                    # print("origin index shape is {}".format(np.array(np.where(data_dict[f+'_pancreas']>0)).shape))
                    index = np.array(np.where(data_dict[f+'_pancreas']>0)).T
                    # after T index shape is (91166, 3)
                    # print("after T index shape is {}".format(np.array(np.where(data_dict[f + '_pancreas'] > 0)).T.shape))
                ...
                label = label[max(center[0]-L//2-pad_width+shift_L,0):min(center[0]+L//2+pad_width+shift_L,label.shape[0]), \
                            max(center[1]-L//2-pad_width+shift_W,0):min(center[1]+L//2+pad_width+shift_W,label.shape[1]), \
                            max(center[2]-L//2-pad_width+shift_H,0):min(center[2]+L//2+pad_width+shift_H,label.shape[2])]
                # ？
                ...
                img = img[max(center[0]-L//2-pad_width+shift_L,0):min(center[0]+L//2+pad_width+shift_L,img.shape[0]), \
                            max(center[1]-L//2-pad_width+shift_W,0):min(center[1]+L//2+pad_width+shift_W,img.shape[1]), \
                            max(center[2]-L//2-pad_width+shift_H,0):min(center[2]+L//2+pad_width+shift_H,img.shape[2])]
...
# no the label cubix box
class TrueCropResize(BaseTransform):
...
class NumpyLoader_Multi_merge(BaseTransform):
    ...
    def __call__(self, input_string):
        ...
        dataset = input_string.split("_")[0]
        data_dict['id'] = ''.join(re.findall(r'\d+', input_string))
        if dataset == "PANCREAS":
            fomatkeys = "nih" + "_"
            after_kys = int(data_dict['id'])
            if after_kys < 100:
                if after_kys<10:
                    after_kys = "00" + str(after_kys)
                else:
                    after_kys = "0" + str(after_kys)
            else:
                after_kys = str(after_kys)
            ...
            fomatkeys = input_string
            after_kys = ""

        data_dict['name'] = fomatkeys + after_kys


        # print("input_string is {}".format(input_string))    # input_string is PANCREAS_0038/merge.npy
        ...
        for f in self.fields:
            ...
            # data_dict = {
            # "id"： PANCREAS_0038/merge.npy
            # "venous"： input_image
            # "venous_pancreas": label
            # }
            if self.load_pseudo:
...
class NumpyLoader_Multi_merge_implicit(BaseTransform):
    """
    Loads an image directly to np.array using npy files
    """
    ...
    def __call__(self, input_string):
        ...
        if dataset == "PANCREAS":
            ...
            implicitpath = "/data/MING/data/NIH/Dissm/CT/msd/sdf_folder"
            fomatkeys = "msd" + "_"
            after_kys = int(data_dict['id'])
            if after_kys < 100:
                if after_kys < 10:
            ...
            implicitpath = "/data/MING/data/NIH/Dissm/CT/word/sdf_folder"
            fomatkeys = "word" + "_"
            after_kys = int(data_dict['id'])
            if after_kys < 100:
                if after_kys < 10:
            ...
            implicitpath = "/data/MING/data/NIH/Dissm/CT/syn/sdf_folder"
            fomatkeys = input_string
            after_kys = ""
        ...
        for field in self.fields:
            ...
            if isinstance(data_dict.get(field) ,np.ndarray):
                if self._reshape_view is not None:
                    # field venous_pancreas or venous_origin or  venous
...
def vis_image(dataset, data_path, label_path, data_id, organ="pancreas"):
    """
    dataset:指明哪个数据集
    data_path:数据集路径
    label_path: 标签路径
    ...
    if dataset== "msd":
        ...
        fomatkeys = "amos_"
        label_fomatkeys = fomatkeys
    data_files = fomatkeys + data_id
    label_files = label_fomatkeys + data_id
    # 2. 加载文件和label
    image = nib.load(os.path.join(data_path, data_files + ".nii.gz"))
    label = nib.load(os.path.join(label_path, label_files + ".nii.gz"))
    ...
    # 3. clip 和 scalintenti（图像）label不用
    ...
    crop_label1 = \
        ...
        max(0, center[1] - int(L / 2)):min(label.shape[1], center[1] - int(L / 2) + L), \
        max(0, center[2] - int(L / 2)):min(label.shape[2], center[2] - int(L / 2) + L)]
    # 6. 获取第二个bounding box，同时保存开始和结束的索引（可以不pad）
    ...
    crop_label2 = crop_label1[
            max(center[0] - L // 2 - pad_width + shift, 0):min(center[0] + L // 2 + pad_width + shift, crop_label1.shape[0]), \
            max(center[1] - L // 2 - pad_width + shift, 0):min(center[1] + L // 2 + pad_width + shift, crop_label1.shape[1]), \
            max(center[2] - L // 2 - pad_width + shift, 0):min(center[2] + L // 2 + pad_width + shift, crop_label1.shape[2])]
    diff = list(L + pad_width * 2 - np.array(crop_label2.shape))
    axis_pad_width = [(int(cur_diff / 2), cur_diff - int(cur_diff / 2)) for cur_diff in diff]
    pad_label = np.pad(crop_label2, axis_pad_width)
    crop_img2 = crop_img1[max(center[0] - L // 2 - pad_width + shift, 0):min(center[0] + L // 2 + pad_width + shift, crop_img1.shape[0]), \
          max(center[1] - L // 2 - pad_width + shift, 0):min(center[1] + L // 2 + pad_width + shift, crop_img1.shape[1]), \
          max(center[2] - L // 2 - pad_width + shift, 0):min(center[2] + L // 2 + pad_width + shift, crop_img1.shape[2])]
    diff = list(L + pad_width * 2 - np.array(crop_img2.shape))
    axis_pad_width = [(int(cur_diff / 2), cur_diff - int(cur_diff / 2)) for cur_diff in diff]
    pad_img = np.pad(crop_img2, axis_pad_width)
    # 7. resize 到128
    re_img = resize(pad_img, (128, 128, 128))
    re_label = resize(pad_label, (128, 128, 128))
    # return 128img, crop1 imag, index, crop2img, index2, all image
    return re_img, crop_img1, crop_index1, crop_img2, crop_index2, image,label
